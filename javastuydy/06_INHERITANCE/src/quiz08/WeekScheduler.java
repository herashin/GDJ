...
public class WeekScheduler {
		...
		for(int i = 0; i < week.length; i++)
		{
			if( dayName.equals(dayNames[i])  )
			{
				if(week[i]  == null) // 등록된 스케줄이 없으면
				{
					...
					String schedule = sc.nextLine(); // nextLine을 쓰면 공백입력이 가능함
					Day day = new Day();
		...
		for( int i = 0; i < week.length; i++ )
		{
			if( dayName.equals(dayNames[i]) )
			{
				if( week[i] == null  )
				{
					...
					System.out.println(dayName + "요일의 스케쥴은 " + week[i].getSchedule());
	...
	public void manage() {
		...
		default:System.out.println("인식할 수 없는 명령입니다.");break;
		}
		}
