...
SELECT 
   ...
   ,SUM(ENG) AS 영어
   ,SUM(MATH) AS 수학
--   ,SUM(KOR, ENG, MATH) 인수(arguments)가 3개이므로 불가능함(인수 1개여야 함) AS D
   ,SUM(KOR + ENG + MATH) AS 총합  -- KOR + ENG + MATH와 같은 연산(SUM 함수를 ???
   ,SUM(KOR) + SUM(ENG) + SUM(MATH) AS 총합2 -- 국어합 + 영어합 + 수학 합
