...
/*
    ...
    2. TO_CHAR(데이터, '형식') : 지정된 데이터(주로 숫자나 날짜)를 형식에
...
SELECT
   ...
   ,TO_CHAR(1.5, '9') -- '2', 형식으 정수 1자리 표기(소수 이하 반올림)
...
SELECT EMP_NO, NAME , DEPART, POSITION , GENDER, HIRE_DATE, SALARY AS S
    ...
    WHERE TO_DATE(HIRE_DATE, 'YY-MM-DD') BETWEEN TO_DATE('90/01/01' , 'YY/MM/DD') AND TO_DATE('31/12/99', 'DD/MM/YY');
     -- 실무에서 날자는 문자로 처리하는 경우가 많다 
    
    
CREATE TABLE SIMSAMPLE(
    DT1 DATE,
    DT2 TIMESTAMP,
...
-- DT1 과 DT2 칼럼에 현재 날짜 넣기
INSERT INTO 
SIMSAMPLE(DT1, DT2, DT3)
VALUES(SYSDATE, SYSTIMESTAMP, TO_CHAR(SYSDATE, 'YYYY-MM-DD'));

SELECT DT1,DT2,DT3 FROM SIMSAMPLE;
