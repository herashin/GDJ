...
SELECT EMP_NO, NAME
    FROM EMPLOYEE
   WHERE ROWID =  'AAAFB9AABAAALDBAAC'; 
...
FROM EMPLOYEE (SELECT EMP_NO,NAME 
                FROM EMPLOYEE
...
--1:    SELECT
--2:    FROM(ROWNUM의 별명 지정)
--3:    WHERE ROWNUM의 별명 사용하기 
...
SELECT ROWNUM, B.EMP_NO, B.NAME, B.SALARY
    FROM ( SELECT EMP_NO,NAME, SALARY
             FROM EMPLOYEE
...
-- 1) ROWNUM 칼럼
SELECT D.ROW_NUM, D.EMP_NO , D.NAME, D.SALARY
    FROM( SELECT ROWNUM AS ROW_NUM, C.EMP_NO, C.NAME, C.SALARY
            FROM ( SELECT EMP_NO, NAME, SALARY
                    FROM EMPLOYEE
                    ORDER BY SALARY DESC) C) D
  --  WHERE D.ROW_NUM = 2; 
      WHERE D.ROW_NUM BETWEEN 2 AND 2; 
...
-- 3) RANK() 함수
-- 정렬 후 순위 매기는 함수
-- 목록 가져오기에서는 부적적(동점자 처리 떄문에 가져오는 목록의 수가 매번 달라질 수 있음)
SELECT A.EMP_NO, A.NAME , A.SALARY
...
-- 2. 테이블 복사할 때 사용
-- 3. NOT NULL 제약조건을 제외 제약조건은 복사되지 않음
-- 4. 형식
...
UPDATE EMPLOYEE2
    ...
    ,GENDER = (SELECT NAME GENDER FROM EMPLOYEE WHERE EMP_NO = 1004)
WHERE EMP_NO = 1001;
